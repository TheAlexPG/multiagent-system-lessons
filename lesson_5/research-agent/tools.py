...
from config import MAX_SEARCH_RESULTS, OUTPUT_DIR, READ_URL_MAX_CHARS, KNOWLEDGE_MAX_CHARS


def knowledge_search(query: str) -> str:
    """Search the local knowledge base (ingested documents). Returns formatted results."""
    try:
    ...
    for i, r in enumerate(results, 1):
        text = r["text"]
        if total_chars + len(text) > KNOWLEDGE_MAX_CHARS:
            text = text[:KNOWLEDGE_MAX_CHARS - total_chars] + " [truncated]"
        total_chars += len(text)

        lines.append(
            f"--- Result {i} [source: {r['source']}, "
            f"rerank: {r['score_rerank']}, "
            f"semantic: {r['score_semantic']}, "
            f"bm25: {r['score_bm25']}] ---\n"
            f"{text}\n"
        )

        if total_chars >= KNOWLEDGE_MAX_CHARS:
            break
